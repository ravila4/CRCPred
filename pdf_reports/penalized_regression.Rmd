...
setwd("~/Documents/school/postgenomics/project")
...
for (i in 0:10)
{
  assign(paste("fit", i, sep=""), cv.glmnet(x.train, y.train, type.measure="auc", alpha=i/10, 
...
for (p in models) {
  accuracy <- 1 - sum(abs(y.test - as.integer(p))) / nrow(x.test)
...
for (p in models) {
  accuracy <- 1 - sum(abs(y.test - as.integer(p))) / nrow(.test)
  roc_obj <- roc(y.test, as.integer(p))
  print(paste("Accuracy:", accuracy, "AUC:", auc(roc_obj)))
}
```
